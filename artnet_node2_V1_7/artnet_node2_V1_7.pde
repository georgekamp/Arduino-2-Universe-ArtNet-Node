...
uint8_t factory_broadcastIp  [4] = {  broadIp_a, broadIp_b,  broadIp_c, broadIp_d};
...
void updateLED() {
 ...
 // if (fadeToState != 0) {
    if (!online) {
      digitalWrite(redPin, HIGH);                                   //analogWrite(redPin, 255 - fadeToState);
      digitalWrite(greenPin, LOW);                                   //analogWrite(greenPin, fadeToState);
    } else {
      digitalWrite(greenPin, HIGH);                                                             //analogWrite(redPin, fadeToState);
      digitalWrite(redPin, LOW);                                                                        //analogWrite(greenPin, 255 - fadeToState);
    }
    delay(5);
    //fadeToState--;
  }
